...
find_player() {
    # Try any available player first
    local active_player=$(playerctl -p "$PLAYERS" status 2>/dev/null)
    if [[ "$active_player" == "Playing" || "$active_player" == "Paused" ]]; then
        playerctl -p "$PLAYERS" metadata --format "{{ mpris:playerName }}" 2>/dev/null
        return 0
    fi
